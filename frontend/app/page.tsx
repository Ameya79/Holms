...
export default function LandingPage() {
  ...
  return (
    <main className="min-h-screen flex flex-col bg-sand text-ink font-sans selection:bg-match selection:text-ink">
      ...
      {/* Hero Section */}
      <section className="relative pt-12 pb-12 flex flex-col items-center justify-center text-center px-6 overflow-hidden">
        {/* Palm Island Logo Badge */}
        <div className="mb-6 relative">
          <Logo size={72} className="shadow-sm border border-teal/20 p-1 bg-foam rounded-2xl" />
        </div>

        <h1 className="font-serif text-5xl sm:text-7xl text-teal tracking-wide mb-4 max-w-3xl leading-[1.08]">
          Your documents. <br />
        ...
        {/* Feature Component (Orbit Tech Stack Feature Section in Hero) */}
      ...
      <div className="bg-foam border-y border-teal/15 py-5 px-6">
        <div className="max-w-4xl mx-auto flex flex-wrap items-center justify-center gap-6 text-teal font-medium text-xs sm:text-sm text-center">
          <span className="flex items-center gap-2">
            <Lock className="h-3.5 w-3.5 text-drift" />
            Search what's yours
          </span>
          ...
          <span className="text-drift/40 hidden sm:inline">•</span>
          <span className="flex items-center gap-2">
            <Shield className="h-3.5 w-3.5 text-drift" />
            Nothing leaves your machine
          </span>
        </div>
      </div>

      {/* Visual Infographics Component */}
      <Infographics />
